...
class NerfModel(nn.Module):
  ...
  distance_scale: float  # Distance scaling factor for sigma activation.
  density_offset: float # Density offset for sigma activation.
  gamma: float  # Scalar on raw output from density network.
  ...
  def __call__(self, rng_0, rng_1, rays, randomized):
    ...
    sigma = self.sigma_activation(raw_sigma * self.gamma + self.density_offset)
...
def construct_nerf(key, example_batch, args):
  ...
  if args.sigma_activation == "relu":
    sigma_activation = nn.relu
    inverse_gamma = 1.
    density_offset = 0.
  elif args.sigma_activation == "exp":
    sigma_activation = jnp.exp  # utils.safe_exp
    inverse_gamma = 1. / args.gamma # gamma is inputted to be some whole number like 1, 2, 4, 8, 16
    density_offset = jnp.log(jnp.log(1 / 0.99)) - jnp.log(4. * args.distance_scale) - 0.05  # haochen parameterization
    # density_offset = (jnp.log(jnp.log(1 / 0.99)) - jnp.log(4. * args.distance_scale) - 0.05 * inverse_gamma**2) / inverse_gamma # josh parameterization
  else:
    raise ValueError("Invalid sigma_activation: {}".format(args.sigma_activation))
  print(f"parametrizing sigma with {args.sigma_activation}, density_offset: {density_offset} w/ L={4*args.distance_scale} and gamma={inverse_gamma}")
