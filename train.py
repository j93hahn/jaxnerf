...
def main(unused_argv):
  ...
  with EventStorage() as metric:
    for step, batch in zip(range(init_step, FLAGS.render_every + 2), pdataset):
      metric.step()
      # if reset_timer:
      ...
      if jax.host_id() == 0:
        if step % FLAGS.print_every == 0:
          ...
          metric.put_scalars(loss=float(avg_loss), psnr=float(avg_psnr))
      ...
      if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
        ...
        print("beginning full evaluation on the test set...")
        psnrs = []
        ...
        for i in range(200):
          ...
          if jax.host_id() == 0:
            ...
            psnrs.append(psnr)
            # render predicted image and ground truth side-by-side and save
            # to disk
            x = np.array(pred_color)
            y = np.array(test_case["pixels"])
            combined_image = np.concatenate((x, y), axis=1) # ground truth is on the right
            combined_image = (combined_image * 255).astype(np.uint8)  # prevent `TypeError: Cannot handle this data type: (1, 1, 3), <f4`
            iio.imwrite(
                f"eval/pose_{i}.png",
                combined_image
            )
        ...
        if jax.host_id() == 0:
          psnrs = jnp.array(psnrs)
          test_psnr = jnp.mean(psnrs)
          # psnr = jax.device_get(psnr)
          eval_time = time.time() - t_eval_start
          print(f"Eval {step}: {eval_time:0.3f}s., Full test-set PSNR: {test_psnr:0.3f}")
          with open('psnrs.txt', 'a') as f:
            f.write(f"{test_psnr:0.3f}\n")
  ...
  checkpoints.save_checkpoint(  # save final checkpoint in current directory
      os.getcwd(), state, FLAGS.render_every + 2, keep=100)
